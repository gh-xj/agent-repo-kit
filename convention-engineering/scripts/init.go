...
func scaffoldTrackedRepo(root, repoName string, opts initOptions) error {
	...
	if err := ensureConventionSupportFiles(root, opts); err != nil {
		return err
	}
...
func resolveTemplatesRoot() (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("failed to resolve init source path")
	}
	root := filepath.Clean(filepath.Join(filepath.Dir(thisFile), "..", "references", "templates"))
...
func ensureConventionSupportFiles(root string, opts initOptions) error {
	if err := os.MkdirAll(filepath.Join(root, ".convention-engineering"), 0o755); err != nil {
		return err
	}
	if err := writeManagedTextFile(filepath.Join(root, initConventionsCheckPath), renderConventionCheckScript(), 0o755); err != nil {
		return err
	}
...
func renderConventionCheckScript() string {
...
resolve_tool_dir() {
  ...
  printf 'convention-engineering tool not found. Set CONVENTION_ENGINEERING_DIR or install agent-repo-kit into ~/.claude/skills or ~/.codex/skills.\n' >&2
  return 1
}
