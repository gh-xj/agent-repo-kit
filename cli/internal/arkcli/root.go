// Package arkcli wires the `ark` command-line surface.
//
...
type CLI struct {
	...
	Upgrade     UpgradeCmd     `cmd:"" help:"upgrade the running ark/work binaries in place"`
...
func execWriters(args []string, stdout, stderr io.Writer) int {
	...
	return cliruntime.Execute(cliruntime.Options{
		Meta: cliruntime.Meta{
			Name:        binaryName,
			Description: "agent-repo-kit unified CLI",
			Version:     appVersion,
			Commit:      appCommit,
			Date:        appDate,
		},
