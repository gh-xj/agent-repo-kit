...
if ls "$PAGES_DIR"/*.md >/dev/null 2>&1; then
  for f in "$PAGES_DIR"/*.md; do
    ...
    if [ -z "$fm_id" ]; then
      err "$f: missing frontmatter id  → fix: add \"id: $expected_id_in_name\" to the YAML frontmatter"
      continue
    fi
    if [ "$fm_id" != "$expected_id_in_name" ]; then
      err "$f: frontmatter id '$fm_id' does not match filename id '$expected_id_in_name'  → fix: rename file to '$fm_id-<slug>.md' or change frontmatter id to '$expected_id_in_name'"
    fi
    ids+=("$fm_id")
  done
fi
...
if [ "${#ids[@]}" -gt 0 ]; then
  dupes=$(printf '%s\n' "${ids[@]}" | sort | uniq -d)
  if [ -n "$dupes" ]; then
    while IFS= read -r d; do err "duplicate id: $d  → fix: IDs are never reused; assign a new id to one of the pages"; done <<<"$dupes"
  fi
fi
    ...
    if [ -n "$refs" ]; then
      while IFS= read -r ref; do
        ...
        if ! ls "$PAGES_DIR/$ref-"*.md >/dev/null 2>&1; then
          err "$f: wikilink [[$ref]] has no target page  → fix: create '$PAGES_DIR/$ref-<slug>.md', remove the link, or fix the typo"
        fi
      done <<<"$refs"
    fi
  done
...
if ls "$PAGES_DIR"/*.md >/dev/null 2>&1; then
  for f in "$PAGES_DIR"/*.md; do
    ...
    if [ -z "$raw" ]; then
      err "$f: source page missing raw_path  → fix: add 'raw_path: raw/<filename>' to frontmatter (required for source pages)"
    elif [[ "$raw" != raw/* ]]; then
      err "$f: raw_path '$raw' must start with 'raw/'  → fix: move the source file under raw/ and update raw_path"
    elif [ ! -e "$raw" ]; then
      err "$f: raw_path '$raw' does not exist  → fix: create the file at that path, or update raw_path to point to an existing file"
    fi
  done
fi
